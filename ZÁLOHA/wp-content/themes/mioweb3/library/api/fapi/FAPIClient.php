...
if (!class_exists('FAPIClient')) {
	class FAPIClient_Pest
	{
		...
		public $curl_opts = [CURLOPT_RETURNTRANSFER => true, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_FOLLOWLOCATION => false, CURLOPT_MAXREDIRS => 10];
		...
		public function __construct($base_url)
		{
			...
			$this->curl_opts[CURLOPT_HEADERFUNCTION] = [$this, 'handle_header'];
		}
		...
		protected function prepRequest($opts, $url)
		{
			...
			$this->last_request['method'] = $opts[CURLOPT_CUSTOMREQUEST] ?? 'GET';
		...
		private function doRequest($curl)
		{
			...
			$this->last_response = ['body' => $body, 'meta' => $meta];
			curl_close($curl);
	...
	class FAPIClient
	{
		...
		public function __construct($username, $password, $url = 'https://api.fapi.cz')
		{
			$this->RESTClient = new FAPIClient_PestJSON($url);
			$this->RESTClient->setupAuth($username, $password);
			$resources = ['client', 'invoice', 'item', 'periodicInvoice', 'currency', 'paymentType', 'country', 'settings', 'email', 'log', 'user', 'itemTemplate'];
		...
		public function processException(FAPIClient_Pest_Exception $exception)
		{
			...
			$exception = new $class($json->message ?? null, 0, $exception);
	...
	class FAPIClient_InvoiceResource extends FAPIClient_Resource
	{
		...
		{
			try {
				$response = $this->client->post($this->url . '/send-email', [
					'invoice' => $id,
