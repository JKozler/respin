...
class Json
{
	...
	private static array $messages = [
		...
		// exists since 5.3.3, but is returned since 5.3.1
	...
	public static function encode($value, int $options = 0): string
	{
		$flags = \PHP_VERSION_ID >= 50400
			? (\JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES | ((bool) ($options & self::PRETTY)
					? \JSON_PRETTY_PRINT
					: 0))
			: 0;

		if (\PHP_VERSION_ID < 50500) {
			$json = Callback::invokeSafe('json_encode', [$value, $flags], static function ($message): void {
		...
		if ((bool) $error) {
			$message = static::$messages[$error] ?? (\PHP_VERSION_ID >= 50500
					? \json_last_error_msg()
					: 'Unknown error');
	...
	public static function decode(string $json, int $options = 0)
	{
		...
		if (!$forceArray && (bool) \preg_match('#(?<=[^\\\\]")\\\\u0000(?:[^"\\\\]|\\\\.)*+"\s*+:#', $json)) {
		...
		if ($value === null && $json !== '' && (bool) \strcasecmp($json, 'null')) {
